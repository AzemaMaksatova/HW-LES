...
const int BOT_R = 5, BOT_G = 7, BOT_B = 6;
...
void updateFrontLEDs() {
  if (millis() - lastBlink > 500) {
    ...
    if (blinkTop)
      setRGB(TOP_R, TOP_G, TOP_B, blinkState ? 255 : 0, blinkState ? 255 : 0, 0);
    else if (!headlights)
      setRGB(TOP_R, TOP_G, TOP_B, 0, 0, 0);

    if (blinkBot)
      setRGB(BOT_R, BOT_G, BOT_B, blinkState ? 255 : 0, blinkState ? 255 : 0, 0);
    else if (!headlights)
      setRGB(BOT_R, BOT_G, BOT_B, 0, 0, 0);
  }
